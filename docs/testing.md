...
| Focused production-path regressions | Passed | On 2026-09-21, `node --check src/alpha-jump.js` and `node --test tests/alpha-jump.test.js` passed: 7 passed, 0 failed. A minimal fake DOM drives the actual context, complete-render gate, confirmed native-subset gate, picker capture/native-clear, `execute`, `waitForReady`, cancellation, query-change, and detach code paths. It does not establish browser compatibility. |
...
| Native clear | Start with an active native alphabet, including a same-card result; verify ordinary clear, replacement readiness, and retained non-alphabet filters. | Not run in the browser. The newest local source permits a native subset only after the same non-alphabet query was already observed as a complete >100-card rendered result; deterministic coverage passes. The currently served `AJ test` predates this change, so paste the latest source, save, reload, then test it. An initially ambiguous <=100 native subset remains intentionally native. |
